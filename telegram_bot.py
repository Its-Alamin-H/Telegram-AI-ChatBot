...
async def start_cmd(message: types.Message):
    try:
        ...
        messages[username] = []
        await message.answer("Hello, I'm bot powered on GPT-4 model created by Alamin H.")
...
async def new_topic_cmd(message: types.Message):
    try:
        ...
        messages[str(userid)] = []
        await message.reply('Starting a new topic! * * * *', parse_mode='Markdown')
    except Exception as e:
...
async def send_image(message: types.Message):
    try:
        ...
        if not description:
            await message.reply('Please add a description of the image after the /image command. For example, /image Neon City * * ',parse_mode='Markdown')
            return
    except Exception as e:
    ...
    try:
       # Send a "processing" message to indicate that the bot is working
        ...
        await message.reply(
            f"An error occurred during image generation: * *  {e} * * *")
...
async def echo_msg(message: types.Message):
    try:
        ...
        if user_message.startswith("/"):
            # Remove the "/ask" prefix from the user's message
            user_message = user_message[len("/"):].strip()
